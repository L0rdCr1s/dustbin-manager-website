...
class DriverNavigatorRegistration extends React.Component {
    ...
    validateUser = event => {
      ...
      fetch('http://localhost:8000/api/validatefromiaa', {
        method: 'POST',
        headers: {
    ...
    register = event => {
      ...
      if (password === password2) {
        // checking if the user is already registered by IAA
        fetch('http://rally.co.tz/api/dn_registration', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
    ...
    return (
      <React.Fragment>
        ...
        :
                ...
                <a href="../login" className="btn btn-lg btn-block btn-pink btn-see-more mb-3 text-white"
                 style={{padding: 8 + "px"}}>
                    Go to login
                </a> :
